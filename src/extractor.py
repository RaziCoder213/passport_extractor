...
class PassportExtractor:
    ...
    def __init__(self, use_gpu=USE_GPU, languages=None, airline=None):
        ...
        self.airline = airline  # future airline-specific logic
        ...
        logger.info(
            f"Initializing EasyOCR Reader (GPU={use_gpu}, airline={airline})"
        )
    ...
    def extract_mrz_from_roi(self, img_path):
        try:
            ...
            if not mrz:
                logger.warning("PassportEye failed — trying rotations")
                mrz = self._retry_with_rotation(img_path)

            if not mrz:
                logger.warning("Rotation failed — using full OCR fallback")
    ...
    def get_data(self, img_path):
        ...
        data = {
            "surname": mrz.surname.replace("<<", " ").strip().upper()
            if mrz.surname else "",
            "name": mrz.names.replace("<<", " ").strip().upper()
            if mrz.names else "",
            "sex": get_sex(mrz.sex),
            "date_of_birth": parse_date(mrz.date_of_birth)
            if mrz.date_of_birth else "",
