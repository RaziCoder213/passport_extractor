...
class PassportExtractor:
    ...
    def extract_given_name_from_visual_zone(self, img_path):
        ...
        try:
            ...
            if given_name:
                ...
                # Remove any trailing single letters or numbers that might be OCR artifacts
                given_name = re.sub(r'[A-Z0-9]$', '', given_name)
                # Remove any non-alphabetic characters from the end
                given_name = re.sub(r'[^A-Z\s]$', '', given_name, flags=re.IGNORECASE)
                # Remove any remaining field labels that might be at the end
                given_name = re.sub(r'\s+(OF|THE|AND|OR)$', '', given_name, flags=re.IGNORECASE)
    ...
    def get_data(self, img_path, airline=None):
        ...
        # Try to extract given name from visual inspection zone first
        ...
        # Use visual given name if available, otherwise fall back to MRZ
        if visual_given_name:
            name = visual_given_name
            logger.info(f"Using given name from visual zone: {name}")
        else:
            name = clean_name_field(getattr(mrz, 'names', getattr(mrz, 'name', '')))
            logger.info(f"Using given name from MRZ: {name}")
